...
public class HsimContinuous {
    ...
    public DataSet hybridsimulate() {
        ...
        //create separate parallel tasks for chunks of rows. how many rows, based on parallelism()
...
//========================================PRIVATE METHODS====================================//
    ...
    class SimulateRowsTask  {
        SemIm subgraphIM;
        Set<Node> mbAll;

        private SimulateRowsTask(final SemIm subgraphIM, final Set<Node> mbAll) {
            this.subgraphIM = subgraphIM;
            this.mbAll = mbAll;

        }

        protected void compute(List<Integer> rows) {
    ...
    // ************** SYNCHRONIZED METHOD FOR UPDATING SHARED OBJECT ***********************
    private synchronized void updateData(int row, Node node, DataSet newValues) {
